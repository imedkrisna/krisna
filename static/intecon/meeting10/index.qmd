...
subtitle: "ECES905205 pertemuan 10"
date: "11 Mei 2026"
...
den_fill <- if (have_ggden && exists("scale_fill_den", mode = "function")) {
  scale_fill_den()
} else {
  scale_fill_brewer(palette = "Set2")
}

gdp <- read_excel("gdp.xlsx", sheet = "Sheet1") %>%
  filter(year >= 2022, year <= 2025) %>%
  pivot_longer(c(C, I, G, X, M), names_to = "komponen", values_to = "value") %>%
  mutate(
    share = value / GDP,
    komponen = factor(komponen, levels = c("C", "I", "G", "X", "M"))
  )

ggplot(gdp, aes(x = factor(year), y = value, fill = komponen)) +
  geom_col() +
  geom_hline(yintercept = 0, colour = "grey30") +
  geom_text(aes(label = percent(share, accuracy = 0.1)),
            position = position_stack(vjust = 0.5),
            colour = "white", size = 3.3) +
  scale_y_continuous(labels = label_number(scale = 1e-3, suffix = " trln")) +
  den_fill + den_theme +
  labs(x = NULL, y = "Trilyun Rp (konstan 2010)",
       caption = "sumber: BPS, Seri 2010 PDB Penggunaan")
```
...
:::: columns
::: {.column width="50%"}
![source: World Economic Outlook April 2026 IMF](globalcab.png)
:::
