---
title: "Cost curve of the firm"
subtitle: "Week 5"
author: "Krisna Gupta"
date: "8 March 2021 (updated: `r Sys.Date()`)"
output:
...
<iframe src="https://giphy.com/embed/dJZi221XtSnHW" width="480" height="273" frameBorder="0" class="giphy-embed" allowFullScreen></iframe><p><a href="https://giphy.com/gifs/dog-lawnmower-yard-work-dJZi221XtSnHW">via GIPHY</a></p>
...
kensi<-data.frame(no,lab,rice,FC,VC,TC)
kensi %>%
...
```{r kensi1,echo=FALSE, message=FALSE, warning=FALSE}
kensi1<-graph+
  geom_line(data=kensi,aes(x=rice,y=TC),color='red')+
  geom_point(data=kensi,aes(x=rice,y=TC),color='white')+
  annotate("text",x=kensi$rice,y=kensi$TC-50,label=kensi$no,size=8)
kensi1
...
.s[Liv is a learning entrepreneur. She start her fancy footwear business in New York with this cost structure:]

```{r liv1,echo=FALSE, message=FALSE, warning=FALSE}
q<-seq(0,10,1)
FC<-rep(108,11)
VC<-12*q*q
TC<-FC+VC
...
.pull-right[.s[
- For the first fancy footwear produced, liv's $\Delta$Q is 1, while her $\Delta$TC is 120-108=12
...
```{r grafikliv1,echo=FALSE, message=FALSE, warning=FALSE,fig.height=9}
gliv<- graph+
  geom_point(data=liv,aes(x=q,y=TC),color='white',size=4)+
  geom_line(data=liv,aes(x=q,y=TC),color='orange',size=2)+
  scale_x_continuous(breaks=liv$q)+
  scale_y_continuous(breaks=liv$TC)+
...
```{r grafikliv2,echo=FALSE, message=FALSE, warning=FALSE}

gliv2<- graph+
  geom_point(data=liv,aes(x=q,y=MC),color='white',size=4)+
  geom_line(data=liv,aes(x=q,y=MC),color='red',size=2)+
  scale_x_continuous(breaks=liv$q)+
  labs(x='Q (pair)',y='Cost ($)')
...
```{r liv2,echo=FALSE, message=FALSE, warning=FALSE}

liv$AFC<-liv$FC/liv$q
liv$AVC<-liv$VC/liv$q
liv$ATC<-liv$TC/liv$q

liv1<-data.frame(liv$q,liv$TC,liv$AFC,liv$AVC,liv$ATC)
...
.pull-left[
```{r liv3,echo=FALSE, message=FALSE, warning=FALSE}
...
```{r liv4,echo=FALSE, message=FALSE, warning=FALSE}
liv2<-liv[!(liv$q<1),]
gliv3<- graph+
  geom_point(data=liv2,aes(x=q,y=ATC),color='white',size=4)+
  geom_line(data=liv2,aes(x=q,y=ATC),color='red',size=2)+
  scale_x_continuous(breaks=liv2$q)+
  ylim(0,130)+
...
```{r liv5,echo=FALSE, message=FALSE, warning=FALSE, fig.width=10}
gliv4<- graph+
  geom_line(data=liv2,aes(x=q,y=ATC,color='ATC'),size=2)+
  geom_line(data=liv2,aes(x=q,y=AFC,color='AFC'),size=2)+
  geom_line(data=liv2,aes(x=q,y=AVC,color='AVC'),size=2)+
  geom_line(data=liv2,aes(x=q,y=MC,color='MC'),size=2)+
  scale_x_continuous(breaks=liv2$q)+
  labs(x='Q (pair)',y='Cost ($)')
...
```{r liv6,echo=FALSE, message=FALSE, warning=FALSE,fig.width=10}

liv2$ATCC<-c(222,120,90,78,73.2,72,72.86,75,78,81.6)

gliv5<- graph+
  geom_line(data=liv2,aes(x=q,y=ATC,color="Low capacity ATC"),size=2)+
  geom_line(data=liv2,aes(x=q,y=ATCC,color="High capacity ATC"),size=2)+
  scale_x_continuous(breaks=liv2$q)+
  labs(x='Q (pair)',y='Cost ($)')
gliv5
...
.pull-left[![fryer](cafe.jpg)]
.pull-right[![gorengan](gorengan.jpg)]
...
liv3<-data.frame("q"=liv2$q,"TC"=liv2$TC)
liv3$TR<-liv3$q*100
liv3$profit<-liv3$TR-liv3$TC
...
profit1<- graph+
  geom_point(data=liv3,aes(x=q,y=TC),color='white',size=2)+
  geom_line(data=liv3,aes(x=q,y=TC),color='orange',size=1.5)+
  geom_point(data=liv3,aes(x=q,y=TR),color='white',size=2)+
  geom_line(data=liv3,aes(x=q,y=TR),color='cyan',size=1.5)+
...
liv4<-data.frame("q"=liv2$q,"ATC"=liv2$ATC)
liv4$P<-rep(100,10)
liv4$pr<-liv4$P-liv4$ATC

liv4 %>%
  kable("html", align="cc", col.names = c('Q','ATC','P','P-ATC'), digits = 2) %>%
  kable_styling(bootstrap_options = "striped", font_size = 20) %>%
  row_spec(3,background="blue") %>%
...
profit2<- graph+
  geom_line(data=liv3,aes(x=q,y=MC),color='red',size=1.5)+
  geom_line(data=liv3,aes(x=q,y=MR),color='cyan',size=1.5)+
  geom_line(data=liv2,aes(x=q,y=ATC),color='orange',size=1.5)+
