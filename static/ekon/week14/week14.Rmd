...
subtitle: "Week 14"
author: "Krisna Gupta"
date: "24 May 2021 (updated: `r Sys.Date()`)"
output:
...
1. Joy sell shoes in bulk to his friend in Australia. Her friend pays with letter of credit credit (which delays the payment untill the shoes is arrived). The price is Rp 100,000,000.

1. Panca goes on Holiday to Singapore and spend Rp20,000,000. He pays for the holiday with his saving account in Indonesia.

1. Livinia buys Alphabet's stock in NYSE using her Indonesian bank account. The amount is Rp 50,000,000.
...
bopp <- data.frame(
  ...
  "Export"=c("","100m(J)","","100m","","20m (P)","50m (L)","70m","170m"),
  "Import"=c("","","-20m (P)","-20m","-50m (L)","-100m (J)","","-150m","-170m"),
  "Net"=c("","100m","-20m","80m","-50m","-80m","","","0m")
)

bop2<-kbl(bopp, align = "c", format='html',digits=2, format.args = list(big.mark = ","), col.names = c("Component","Credit","Debit","Net"),caption="Simplified Balance of Payment of Joy-Pan-Liv") %>%
  kable_styling(font_size=18) %>%
  column_spec(1, bold = T) %>%
  row_spec(4, bold = T) %>%
  row_spec(9, bold = T) %>%
