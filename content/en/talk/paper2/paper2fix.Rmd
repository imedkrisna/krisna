...
> To what extent tariff and NTMs on firms' import affect firm's productivity?
...
- Key data sources:
  - Tariff: MoF regulations + FTA partners
  - NTM: UNCTAD TRAINS database
  - Firm's characteristics & trade: Survei Industri (SI) + Customs data
...
kable(tab3, format = "html") %>%
  kable_styling(bootstrap_options = "bordered", full_width=F, font_size = 11) %>%
  collapse_rows()
```
...
.center[
Table 3. Tariff from MoF regulations (left) compared to WTO-IDB (right)
]
...
##### Adding size to the regression

- Firms in the customs data are different: bigger, more productive

- if size matters, it can be discovered using size interaction.

- $l_{it}=log(Labour_{it})$ is used as a proxy for size

$$size\_tfp_{it}=tfp_{it}+\gamma_Tt_{it}*l_{it}+\sum_\theta c_{\theta it}*l_{it}$$

- Observations:
    - only customs, 2008-2012, only $k,n>0$
    - 4,971 observations of 1,512 firms and 5 years

- 3 Proxies for TFPs:
    - TFP1 estimated within all SI observations (estimate first, drop later)
    - TFP2 estimated within only customs (drop first, estimate later)
    - $VaL=Log\left(\frac{Value Added}{Labour}\right)$ for robustness
...
kbl(tab7b,'html') %>%
  kable_styling(bootstrap_options = "striped", font_size = 11) %>%
  row_spec(5:6, bold=T,color="black",background="yellow") %>%
  collapse_rows()
```
]
...
- tariff is negatively associated with TFPs, and the magnitude is less profound for bigger firms:
...
- NTM have mixed results: Only TBT and pre-shipment reduces import

##### Employment
- Tariff also reduces employment, but bigger firms have smaller magnitude.
- Not all NTMs show a visible significance:
  - SPS, licensing, and export-related measures have the same directional correlation as tariff.
...
- Targeting CAD may not be ideal
  - the real problem may not actually be trade
      - Foreign borrowing is getting risky
      - bad debt management $\Rightarrow$ costing manufacturers and consumers
  - Intervening with trade is usually inefficient:
      - Halting GVC may lead to even more reliance on commodities
      - CAD may be necessary for growth (at least in short-medium run)
...
- NTMs have mixed results.
  ...
  - May have different relationship for different types of NTMs on different goods
